...
export default {
    ...
    <section v-if="bug" class="bug-edit">
        ...
        <form @submit.prevent="saveBug">
            ...
            <label>
                ...
                <input type="text" v-model="bug.desc"/>
            </label>
            <div class="actions">
  ...
  methods: {
    saveBug() {
      if (!this.bug.title || !this.bug.severity || !this.bug.desc) eventBus.emit('show-msg', { txt: 'All fields must be filled out.', type: 'error' })
      else
        bugService.save({ ...this.bug })
          ...
          .catch((err) => {
              console.log('eroor at front', err);
